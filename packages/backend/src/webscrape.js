...
function compareResults(res1, res2, isMulti) {
    const compareTime = (time1, time2) => {
        if (time1.includes(":") && time1.includes(".")
            && time2.includes(":") && !time2.includes(".")) {
            return -1;
        } else if (time1.includes(":") && !time1.includes(".")
            && time2.includes(":") && time2.includes(".")) {
            return 1;
    ...
    if (res1 === undefined) {
        ...
        let multi1Points = (2 * parseInt(multi1Cubes[0])) - parseInt(multi1Cubes[1]);
        let multi2 = res2.split(" ");
        let multi2Cubes = multi2[0].split("/");
        let multi2Points = (2 * parseInt(multi2Cubes[0])) - parseInt(multi2Cubes[1]);
        ...
        if (multi1Points > multi2Points) {
            ...
            if (multi1[1] !== multi2[1]) {
                return multi1[1].localeCompare(multi2[1], undefined, { numeric: true });
            } else {
                let multi1DNFs = parseInt(multi1Cubes[1]) - parseInt(multi1Cubes[0]);
                let multi2DNFs = parseInt(multi2Cubes[1]) - parseInt(multi2Cubes[0]);
